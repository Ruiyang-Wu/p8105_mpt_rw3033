...
```
Comment:
Manhattan consistently has the highest rental prices. There are general upward trends across all boroughs from 2020 to 2024. Statwn Island is missing some years of value.
...
```
Comment:
States like CA and NY display a wide range of house prices, with significantly higher upper extremes compared to other states. States such as WY and WV exhibit much narrower distributions and lower median house prices, indicating less variability and overall lower prices.
...
```
Comment:
The scatter plot demonstrates a positive correlation between average rental and housing prices across ZIP codes, as highlighted by the upward-sloping trend line. This upward trend suggests that areas with expensive rental markets also tend to have higher property values.

Limitations:
Missing Data: There are missing values for either rental or housing prices for some ZIP codes, which can skew the analysis.
Lack of Borough-Level Grouping: ZIP codes are not grouped by borough, making it harder to compare rental and housing prices across different boroughs and understand regional price differences.
